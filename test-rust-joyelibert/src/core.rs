...
impl<'c, 'm> Decrypt<DecryptionKey, &'c RawCiphertext<'c>, RawPlaintext<'m>> for JoyeLibert {
    fn decrypt(dk: &DecryptionKey, c: &'c RawCiphertext<'c>) -> RawPlaintext<'m> {
        ...
        //tb
        let g = BigInt::mod_pow(&dk.y, &((p - BigInt::one()) / BigInt::from(2_u32).pow(s as u32)), p);
        let mut t_b: HashMap<String, BigInt> = HashMap::new();

        for w in 0..(1 << s) {
            let value = BigInt::mod_pow(&g, &BigInt::from(w), p);
            t_b.insert(value.to_str_radix(15), BigInt::from(w));
        }

        //td
        let mut t_d: Vec<HashMap<String, BigInt>> = vec![HashMap::new(); (n - 1) as usize];
        for j in 0..(n - 1) {
            let hj= BigInt::mod_pow(&dk.y, &((p - BigInt::one()) / BigInt::from(2_u32).pow((k - (j + 1) * s) as u32)), p);
            for i in 0..(1 << s) {
                let value = BigInt::mod_pow(&hj, &BigInt::from(i), p);
                t_d[j as usize].insert(value.to_str_radix(15), BigInt::from(i));
            }
        }
        
        let start_time = Instant::now();

        let c_exp = (p - BigInt::one()) / BigInt::from(2_u32).pow(k as u32);
        let mut c_value = BigInt::mod_pow(c.0.borrow(), &c_exp, p);

        let zero = BigInt::zero();
        for i in 0..(n - 1) {
            let pow_2_k_i_s = BigInt::from(2_u32).pow((k - (i + 1) * s) as u32);
            let z = BigInt::mod_pow(&c_value, &pow_2_k_i_s, p);

            let b = t_b.get(&z.to_str_radix(15)).unwrap_or(&zero);
    
            if let Some(td_j) = t_d.get(i as usize) {
                if let Some(td_value) = td_j.get(&b.to_str_radix(15)) {
                    c_value = (c_value * td_value) % p;
                }
            }
        ...
        let last_b = t_b.get(&c_value.to_str_radix(15)).unwrap_or(&zero);
        if r < s {
            plaintext += last_b << ((n - 1) * s - r);
        } else {
            plaintext += last_b;
        }
